import { ButtonInteraction, ForumChannel, GuildMember } from "discord.js";
...
const spsettimeleft = async (interaction: ButtonInteraction, collections: any, splitted: Array<string>) => {
    ...
    if (stockpileExist) {
        ...
        // create a date 50 hrs from now
        const expireDate = new Date((new Date()).getTime() + 60 * 60 * 1000 * 50)

        let res = await collections.stockpiles.updateOne(
            { name: searchQuery }, 
            { $set: { expireDate: expireDate }, $unset: { upperBound: 1 } }
        )
        ...
        let updatedStockpile = await collections.stockpiles.findOne({_id:stockpileExist._id})
        await updateStockpileMsg(interaction.client,"",updatedStockpile,true)
    }
    else {
...
const sppurgestockpile = async (interaction: ButtonInteraction, collections: any, splitted: Array<string>) => {
    ...
    let configObj = await collections.config.findOne({})
    let stockpiles = await collections.stockpiles.find({}).toArray()
    for (let i =0;i<stockpiles.length;i++){
        let channel = await interaction.guild?.channels.fetch(configObj.stockpileChannelId) as ForumChannel
        let thread = await channel.threads.fetch(stockpiles[i].threadId)
        await thread?.delete()
        await collections.stockpiles.deleteOne({_id:stockpiles[i]._id})
    }
